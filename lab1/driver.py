...
# Ignore the specific warning about past_key_values
warnings.filterwarnings(
    "ignore",
    message="Passing a tuple of `past_key_values` is deprecated and will be removed in Transformers v4.43.0."
)
...
def main():
    ...
    for sample in data[20:21]:
        ...
        # gold = sample['gold']
        text = sample['text']
        # text = "SURELY LUCK HAS BROUGHT US ALSO TO A NEW CHARACTER"
        cost_model.set_audio(audio, sr)
        # environment_gold = Environment(gold, cost_model.get_loss, phoneme_table)
        environment_text = Environment(text, cost_model.get_loss, phoneme_table)

        # print(gold)
        # try:
        #     agent.asr_corrector(environment_gold)
        #     pred = agent.best_state
        #     cost = agent.best_cost
        #     print(cost)
        # except:
        #     pred = None
        ...
        # print(text)        
        try:
            agent.asr_corrector(environment_text)
            pred = agent.best_state
            cost = agent.best_cost
            # print(cost)
        ...
        print(f'Time taken : {time_elapsed}s')
        # corrected_texts.append(pred)
