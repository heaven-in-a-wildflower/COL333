...
class Game:
    def __init__(self, player1_name, player2_name, player1, player2, time: int, board_init: np.array, layers: int, mode: str):
        ...
        if mode == "gui":
            ...
            self.current = tk.Label(root, text="Current:", font=("Arial", 16))
            self.current.pack()

            player1_string = f"{player1.player_string} (Yellow) | Time Remaining {PLAYER_TIME[0]:.2f} s"
            self.player1_string = tk.Label(root, text=player1_string, anchor="w", width=50, font=("Arial", 15))  # Increased font size to 14
            self.player1_string.pack()

            player2_string = f"{player2.player_string} (Red)    | Time Remaining {PLAYER_TIME[1]:.2f} s"
            self.player2_string = tk.Label(root, text=player2_string, anchor="w", width=50, font=("Arial", 15))  # Increased font size to 14
    ...
    def display_coordinates(self, hex_coords, i, j):
        ...
        self.c.create_text(x, y, text=f"({i},{j})", fill="black", font=("Arial", 15))
    ...
    def display_time(self, game_over):
        while not game_over.value:
            ...
            self.player1_string.configure(text=player1_string, anchor="w", width=50, font=("Arial", 15))  # Updated font size here as well
            self.player2_string.configure(text=player2_string, anchor="w", width=50, font=("Arial", 15))  # Updated font size here as well
    ...
    def make_move(self, game_over, pause_timer, current_turn):
        ...
        if not game_over.value:
            ...
            if self.use_gui:
                ...
                self.current.configure(text=f'Current Turn: {self.players[current_turn.value].player_string} ({color})', font=("Arial", 16))
